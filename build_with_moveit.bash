...
DOCKERFILE_NAME="Dockerfile_MoveIt"
...
echo ".*" > "${1}"/.dockerignore
docker build --rm -t $image_plus_tag -f "${1}"/$DOCKERFILE_NAME "${1}" $BUILD_BASE $BUILD_ROS_GAZ && \
docker tag $image_plus_tag $image_name:latest && \
echo "Built $image_plus_tag and tagged as $image_name:latest"
rm "${1}"/.dockerignore

# Create /tmp/.docker.xauth if it does not exist
# This leads into run.bash, which assumes it does
